		     ...
		     Index *ind)
{
  ...
  //weight = term_freq(d_t) * log(totalNumDocs/numDocsContain(term_t)) * weight(q_t)
  //weight = docTermFreq * log(totalNumDocs/numDocsContain(term_t)) * qryTermWeight
  double logResult = std::log(double(ind->docCount()/ind->docCount(termID)));
  return docTermFreq * logResult * qryTermWeight;
}
		     ...
		     Index *ind)
{
  ...
  //weight = (log(term_freq)+1) * log(totalNumDocs/numDocsContain(term_t))*weight(query)
  double logResult = std::log(double(ind->docCount()/ind->docCount(termID)));
  return log(docTermFreq + 1) * logResult * qryTermWeight;
}
		     ...
		     Index *ind)
{
  ...
  /*weight = term_freq/(term_freq + 0.5 + 1.5 * (len(d)/avgDocLen))) * log((totalNUmDocs - numDocsCOntain(term_t) + 0.5)/(numDocsContain(term_t) * 0.5)
	     * (8+weight(q_t))/(7+weight(q_t))
  */
  double termFreqTerm = double(docTermFreq / (docTermFreq + 0.5 + 1.5 * double(ind->docLength(docID)/ind->docLengthAvg())));
  double logResult = std::log(double((ind->docCount() - ind->docCount(termID) + 0.5)/(ind->docCount(termID) + 0.5)));
  double qryWeights = (double(8 + qryTermWeight))/(double(7 + qryTermWeight));
  return termFreqTerm * logResult * qryWeights;
}
